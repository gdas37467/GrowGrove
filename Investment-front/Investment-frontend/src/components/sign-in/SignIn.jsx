  ...
  const handleSubmit = async (e) => {
   ...
   // Add your form submission logic here
    ...
    try {
      ...
      const superUser = response.data.is_staff
      const tfa = response.data.is2fa
     
      localStorage.setItem("token", token);
      if(superUser === true)
      {
      
        localStorage.setItem("lik_rt_ad_loksminis" , superUser);
      }
      
      const is2FA = jwtDecode(tfa)['is2fa'];
      
      if(is2FA == true){
        localStorage.setItem("lbp2mpf_ma",tfa);
      ...
      <div className="flex mt-10">
        ...
        <div className="mt-5 md:mt-32 ml-2 md:ml-5">
          ...
          <form className="text-md font-opsans  font-extrabold" onSubmit={handleSubmit}>
            ...
            <a href="/send-otp" className="flex justify-center mt-2 cursor-pointer hover:text-purple-700" onClick={handleDontHaveAccount}>
              Forgot Password?
            </a>
          </form>
        </div>
